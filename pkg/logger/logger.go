...
func (l *Logger) WithLevel(lvl Level) *Logger { //设置日志等级
	ll := l.clone()
	ll.level = lvl
	return ll
}

func (l *Logger) WithFields(f Fields) *Logger { //设置日志公共字段
	ll := l.clone()
	if ll.fields == nil {
...
func (l *Logger) WithCaller(skip int) *Logger { //设置当前某一层调用栈的信息（程序计数器、文件信息和行号）
	ll := l.clone()
